...
## 步骤101.1 — 趋势图多指标曲线重叠修复（R29.1）

- **操作时间**：2026-08-09
- **状态**：✅ 完成（前端构建通过；运行时效果待 Docker 环境复验）
- **背景**：R29 上线后用户反馈：数据总览趋势图「新添加的指标会和上一个直接重合覆盖」。定位到两个原因：①同单位指标（温度°C/土壤温度°C、湿度%/土壤湿度%）共用同一 Y 轴，数值接近时曲线几乎完全重叠；②mock/空态分支 `setOption` 未使用 `notMerge`，勾选切换时旧序列可能残留叠加。

### 改动清单（仅 Web 端 TrendChart.vue）
1. `buildAxes` 改为「每个选中指标独立一个 Y 轴」（位置左右交替 + offset 50 阶梯），同单位指标不再共轴，曲线不再视觉重叠
2. `unitIndex` 索引由单位改为指标类型（`m.type`），`buildOption`/`buildMock` 两处同步更新
3. `renderEmpty` 与 mock 分支 `setOption` 增加 `{ notMerge: true }`，与真实数据分支一致，杜绝序列残留
4. `grid` 左右留白微调（left 4% / right 10%），容纳多 Y 轴 label

### 验证
- ✅ 前端 `npm run build` 成功
- ⏳ 勾选「温度+土壤温度」「湿度+土壤湿度」等组合的视觉验证待 Docker 环境复验
...
## ??107 ? APP 403 ???Token ???????+ ?? AI Key ?????APP-C06?
...
### ????APP ?? 403?Token ???
- **??**??? JWT ???? 2 ???`application-dev.yml` `jwt.expiration=7200000`????????? 403?APP ?????????????????? 403
- **??**?
  1. `app/.../data/api/ApiClient.java`??? `SessionExpiredListener` ??????????????? 403 ? `TokenManager.clear()` ?????
  2. `app/.../GreenhouseApplication.java`????????? + `ActivityLifecycleCallbacks` ???? Activity?Token ???????????? `LoginActivity`?`FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK`?
  3. `backend/.../application-dev.yml`?JWT ??? 2 ?? ? 7 ??`604800000`???????????

### ?????? AI Key ?????
- **??**?`.env.local` ?? `BAIDU_API_KEY/BAIDU_SECRET_KEY`?????`XUNFEI_APP_ID/API_KEY/API_SECRET`??????? gitignore ????`application-dev.yml` ? `ai.image.provider` / `ai.voice.provider` ?? `${AI_IMAGE_PROVIDER:mock}` / `${AI_VOICE_PROVIDER:mock}` ???????`.env.local` ???? `baidu` / `xunfei`
- **? ???????AI?**?`POST /api/v1/diagnosis/recognize` ?? `recognitionEngine: "??AI"`???????????
- **? ????????**?`POST /api/v1/qa/ask/voice` ?? `asrEngine: "xunfei"`???????? RAG ????? 5 ???????

### ??????????????? 403 ? ???
1. `VoiceQaService.java`?????????????????? Tomcat ????? `transferTo` ???`getBytes()` ????
2. `XunfeiSpeechProvider.java` ?????
   - ???? HTTP POST ? `ws-api.xfyun.cn/v2/aisp`?????? ?? 403
   - ????OkHttp WebSocket ?? `wss://iat-api.xfyun.cn/v2/iat`????????? `GET /v2/iat HTTP/1.1`?`authorization/date/host` ??? `%20` ?????? 8000 ?????status 0/1/2??????? `code` ???? HTTP ? `header.code`?????? `data` ????
   - ???????? `DEBUG` ???????????
...
## 2026-08-28（R37：Web 全站深色玻璃拟态统一 + 趋势图数据标识稳定性修复 + 数据清理与种子数据填充）

- **时间**：2026-08-28
- **范围**：Web 端（全站 UI/主题 + 趋势图 + 用户管理 + 预警配置 + 问答/知识库等）+ 数据库（脏数据清理、种子数据）+ 设备模拟器（扩展至 6 大棚 38 设备）
- **需求背景**：基于页面设计审查结论，经用户确认执行"方案丙"（全站深色玻璃拟态）、趋势图"数据标识不稳定、自己变换"问题排查修复，以及数据库脏数据清理与数据填充。

### 一、全站深色玻璃拟态（方案丙）
1. 设计令牌（CSS Variables）：--bg-page / --bg-panel / --border-soft / --text-primary / --radius-card 等，统一页面底色、面板、文字、圆角、阴影。
2. 引入 Element Plus 官方 dark 主题（element-plus/theme-chalk/dark/css-vars.css + <html class="dark">），并在 main.css 微调：卡片统一玻璃面板、表格透明底+深色斑马纹、对话框/下拉深色高层面板、滚动条深色化。
3. 登录页：紫渐变 → 深蓝渐变 + 玻璃拟态卡片 + 绿色 Grape 品牌图标；主布局顶栏品牌图标替换 emoji 🌱。
4. 管理端各页深色化（AdminDashboard 统计卡/环境聚合/预警总览/天气/系统监控、用户/设备/知识库/语料/专家/棚主/问答/预警/导出等页面），13 个页面硬编码浅色文字批量提亮（#303133→#e0e6ed 等）。
5. 全局中文空状态：App.vue 挂 el-config-provider :locale="zh-cn"，"No Data" 英文空态全站消失。
6. emoji → SVG 图标：🌱/🤖/📍/🔊/📚 等替换为 Element Plus 图标或纯文本。

### 二、趋势图"数据标识不稳定"修复（TrendChart.vue 重写）
- **根因**：图例/轴位置随勾选顺序变化 + ECharts 滚动图例只显示前两项 + 每次轮询 
otMerge 全量重建重置状态 + 预测接口恒空导致"预测"图例挂空项。
- 修复：
  1. 勾选行改为"带色点的自定义图例"（顺序固定为指标定义顺序，色点与曲线同色）；
  2. 轴位置由指标定义序号决定（不再随勾选顺序左右跳变）；
  3. 预测无数据时标注"暂无预测数据"，不再挂空图例项；
  4. 轮询改为增量合并更新（保留图例/状态），模拟数据缓存（不再每 30s 随机重生成）；
  5. 勾选约束：最多 4 个、最少 1 个，越界 toast 提示；选择状态改为组件内部同步（竞态免疫）。

### 三、数据清理与种子数据填充
1. 脏数据清理：3 个真实姓名存为问号的账号（test_user_082 / tech01 / worker02）、约 15 条乱码 QA 记录、4 条聊天垃圾消息、data_authorizations 乱码申请理由。
2. 种子数据：重建技术员 tech01（陈技工）+ 新增员工 3 名（赵小强/刘大柱/孙秀兰）、专家 2 名（王农艺/赵植保）、棚主 1 名（owner02 李建国 + 黄瓜种植大棚）；为大棚 2-6 填充设备各 6 台（4 传感器+2 控制器）；重写 3 条 QA 历史记录。
3. 设备模拟器扩展：devices.json 覆盖 6 大棚 38 设备（id 与数据库一致），重启后全部上线（38/38 ONLINE）。
4. 种子脚本入库：	ools/seed_data.sql。

### 验证
- Web 
pm run build 通过；浏览器逐页截图验证（登录/管理员总览/棚主看板/设备/用户/预警/问答）；趋势图勾选交互实测（上限 4/下限 1/顺序稳定）。
- 已推送 GitHub（commit 0a3ec50，26 文件，+1735/-538）。

---

## 2026-08-28（R38：三个交互问题修复——角色筛选技术员、地区级联选不出、专家授权过期 400）

- **时间**：2026-08-28
- **范围**：Web 端（用户管理/地区级联组件）+ 后端（专家授权申请）
- **需求背景**：用户反馈三个问题：①用户管理角色筛选缺"技术员"；②地区筛选"假的、选不出来"；③专家申请大棚返回 400。

### 问题 1：角色筛选缺"技术员"
- 修复：web/src/views/users/UserList.vue 角色筛选下拉补 <el-option label="技术员" value="TECHNICIAN" />。
- 验证：下拉现为 管理员/棚主/员工/技术员/专家。

### 问题 2：地区级联"选不出来"（假筛选）
- **根因**：RegionCascader.vue 懒加载中，"藁城区"这类没有乡镇/村下级数据的节点永远不是叶子，点击只展开空列表、永远选不中。
- 修复：懒加载时**预查下一级**（省→查市、市→查区县、区县→查乡镇、乡镇→查村），无下级数据的节点直接标记为 leaf 可选中；预查请求加 3 秒超时兜底（失败/超时按"无下级"处理，级联永不卡死）。
- 验证：实测选中"河北省 / 石家庄市 / 藁城区"→ 查询后用户列表正确过滤出 6 条藁城区记录。

### 问题 3：专家申请大棚 400"已有有效授权"
- **根因**：后端 ExpertService.requestAuthorization 检查已有授权时不判断过期时间——expert01 有 2 条已过期历史授权（8-15/8-17 到期）被当成"有效"拦截；而 available 接口却把过期当 NONE，前后矛盾。
- 修复：ExpertService.java 只拦截**未过期**的 APPROVED 记录（expiresAt 为空或晚于当前时间才拦截），已过期允许重新申请；后端重启生效。
- 验证：API 直测 200 创建 PENDING；浏览器完整流程实测提交成功（toast"申请已提交，等待棚主审批"）；available 状态与申请结果一致。
...
## 2026-08-30（R46.1：地区级联选择器修复——无法展开下一级）

- **时间**：2026-08-30
- **范围**：Web
- **需求背景**：用户反馈"数据总览选择框还是不能用"。实测确认：RegionCascader 使用 el-cascader lazy 懒加载展开，点击省/市等父级节点**不触发 lazyLoad**、下一级永远加载不出来（用户管理筛选、数据总览查看范围、所有复用该组件的弹窗均受影响）。R38 曾以 peek 预查标记 leaf 修复"空节点选不中"，但展开本身的问题仍在。

1. **根因**：el-cascader 在部分环境（Element Plus 2.9.1 + 本项目用法）下 lazy 模式展开父级节点时不会调用 lazyLoad 加载子级（实测点击节点/箭头均无新请求）。
2. **修复（feat 40e2950）**：重构 RegionCascader 为**全量加载非 lazy 方案**——一次性构建整棵五级地区树（省→市→区→乡镇→村，各层并行请求），模块级缓存各页面复用；`el-cascader` 用 `:options` 静态数据，展开即为普通树交互；无下级的节点 children 为空数组天然是叶子可直接选中（顺带根治 R38 的"假筛选"）。
3. **验证（feat 40e2950）**：浏览器实测用户管理筛选与数据总览「查看范围」级联——五级逐级展开可选（河北→石家庄→正定→南牛→曹村），选中值完整回填输入框；全树加载一次后各页面复用缓存。

- **验证**：Web Vite 重启（HMR 更新时 Vite 进程偶发退出）后实测全流程通过。
- 已推送 GitHub（11642cb..40e2950）。

---

## 2026-08-31（R46.2：地区格式规范化——手填 + 省市县自动对齐标准名）

- **时间**：2026-08-31
- **范围**：后端 + Web（前端无改动）
- **需求背景**：用户反馈新增用户（owner03）填写的新地址在地区选择框/筛选中显示不出来。排查确认根因有二：① RegionService 地区选项**只从大棚表聚合**（findDistinctProvinces 等），R46 后用户地址自由填写存在 users 表，大棚表查不到自然不出现在级联选项里；② AdminService.listUsers 地区筛选先按大棚表查棚主ID集合，大棚表无匹配时**直接 return 空列表**，用户自身地区匹配被提前掐断。用户进一步指出：手填"河北"与标准名"河北省"语义一致却作为两个选项并列，给出两个方案（全国行政区划库级联选择 / 手填+强制对齐格式），**用户选择方案二**。

1. **根因一修复（地区选项 = 大棚表 ∪ 用户表）**：UserRepository 补 5 个用户地区 distinct 查询（findDistinctUserProvinces/Cities/Districts/Towns/Villages）；RegionService 五个 getXxx 方法改为 union 合并大棚表与用户表两来源（TreeSet 去重排序），新填写的用户地址自动进入地区级联选项。
2. **根因二修复（用户自身地区筛选不被掐断）**：AdminService.listUsers 删除「大棚表无匹配即 return Collections.emptyList()」的提前返回，改为保留空集合继续走 inRegion 双匹配（用户自身五级命中 或 大棚归属命中）。
3. **方案二实施（手填 + 省市县自动对齐）**：内置全国省/市/县三级标准名表 region_standard.json（31省/342市/2978县，约150KB，来自公开行政区划数据，含"去后缀别名"如 河北→河北省、衡水→衡水市、深州→深州市、正定→正定县、北京→北京市、广西→广西壮族自治区）；新增 RegionNormalizer 组件（@PostConstruct 加载，直配/去后缀匹配两级，未命中原样保留避免误改自定义地名）；直辖市（北京/天津/上海/重庆）市级字段按区级规范（"朝阳"→"朝阳区"而非"朝阳市"）；乡镇/村两级不强制（4万+ 数据量且村级多为俗称，保留自由填写）。
4. **接入写入链路**：AdminService.createUser/updateUser、GreenhouseService.createGreenhouse/updateGreenhouse 保存时省市县三级统一过 RegionNormalizer。
5. **存量数据清洗**：clean_regions.py 基于标准名表生成 UPDATE 清洗存量数据（仅 users#20 三处：河北→河北省、衡水→衡水市、深州→深州市，大棚表本身已是标准格式）。
6. **回归测试**：后端 API 验证——省份聚合归一（四川省/山东省/广东省/河北省/河南省，无"河北"重复项）；河北省下 石家庄市|衡水市 合并（大棚+用户来源）；完整五级链路 河北省→衡水市→深州市→方形花园→驱虎要 可逐级展开；按 河北省/衡水市/深州市 筛选精确命中 owner03；按 河北省 筛选命中 owner01/owner02/owner03；创建用户填简写自动存标准名（河北/衡水/深州→河北省/衡水市/深州市）；直辖市 北京/朝阳/望京→北京市/朝阳区/望京；自定义地名 云梦泽/桃花源/乌托邦 原样保留。Web 浏览器实测：数据总览「查看范围」与用户管理「筛选」五级展开正常、新地址可选可回填、按新地址筛选出 owner03。

- **验证**：后端 mvn 编译+重启；Web 浏览器实测（数据总览查看范围 + 用户管理筛选全流程）。
- 待推送 GitHub（40e2950..R46.2）。
