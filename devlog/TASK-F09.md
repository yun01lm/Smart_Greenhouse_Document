...
## 后续变更（R42：我的页按钮文字消失修复，2026-08-28）

| 文件路径 | 修改类型 | 变更说明 |
|----------|----------|----------|
| `bg_menu_item.xml` | 修改 | 纯白底 → 深色毛玻璃（surface + outline），修复深色主题下白底浅字导致的入口文字不可见 |
| `bg_login_card.xml` | 修改 | 登录卡白底 → 毛玻璃（surface_2 + hairline） |

> 验证：模拟器实测入口行深色毛玻璃、文字清晰可见；已推送 GitHub（2258595）。
