...
- 完成日期：2026-07-13
- 关联DEVLOG：步骤14
- **步骤35 编译修复（2026-07-14）**：ChromaRetrievalService.java `parseQueryResponse` 异常处理补充 `catch(Exception)`
- **步骤37 架构整改（2026-07-14）**：
  - 修改 `RagQaService.java`：移除 SYSTEM_PROMPT/TOP_K 硬编码常量，Prompt/temperature/max_tokens/topK 全部从 application.yml 的 `greenhouse.ai.rag.*` 读取
  - 修改 `application-dev.yml`：新增 `greenhouse.ai.rag` 配置段（system-prompt / temperature: 0.7 / max-tokens: 2000 / top-k: 5）
  - 修改原因：Prompt 和模型参数硬编码在代码中，论文无法做 Prompt 工程对比实验
