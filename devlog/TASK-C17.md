...
## 后续变更（R38：专家授权申请排除过期记录，2026-08-28）

| 文件路径 | 修改类型 | 变更说明 |
|----------|----------|----------|
| `backend/.../expert/service/ExpertService.java` | 修改 | `requestAuthorization` 只拦截**未过期**的 APPROVED 授权（expiresAt 为空或晚于当前才拦截），已过期历史授权不再挡新申请（修复 400"已有有效授权"） |

> 根因：available 接口已把过期授权视为 NONE（可申请），但申请接口未校验过期时间，两者矛盾。
> 验证：API 直测 200 创建 PENDING；浏览器完整申请流程提交成功；已推送 GitHub（cc92eea）。
