...
- **步骤37 架构整改（2026-07-14）**：
  - 新建 `FusionConfig.java`：@ConfigurationProperties(prefix="greenhouse.fusion")，包含 EnvWeights/VisualWeights/OverallWeights/defaultThresholds/severityFactors
  - 修改 `EnvironmentHealthCalculator.java`：权重（compliance:0.5/stability:0.3/consistency:0.2）和默认阈值从 FusionConfig 读取
  - 修改 `VisualHealthCalculator.java`：权重（disease:0.6/growth:0.4）和病害严重性因子从 FusionConfig 读取
  - 修改 `HealthAssessmentService.java`：融合比例（envWeight:0.6/visualWeight:0.4）从 FusionConfig 读取
  - 修改 `application-dev.yml`：新增 `greenhouse.fusion` 配置段
  - 修改原因：所有权重/阈值/因子硬编码，论文无法做权重对比实验。配置化后修改 YAML 即可，无需重新编译

## 备注

- **2026-07-15 更新**：`growth_assessments` 表对应的 JPA 实体 `GrowthAssessment.java` 已于步骤46 补全（审计发现缺失，架构整改 P1-3 关闭）。C15 多模态融合中 VisualHealthCalculator 的长势评分（growthWeight=0.4）后续可对接此实体。
