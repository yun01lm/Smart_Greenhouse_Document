...
### RAG 检索链路确认（文字问答）
完整链路：`用户问题 → 农业域守卫预检（R7，非农问题拦截）→ 问题向量化（EmbeddingService，provider=siliconflow）→ Chroma 检索（topK=5）→ 组装上下文（【参考资料N】）→ DeepSeek 生成（LlmProvider）→ 保存问答记录并返回引用来源`

### 实现说明
- 检索来源：`qa_records.sources` 存储 JSON 数组（title/category），前端可展示引用来源
- 降级策略：Chroma 无结果或不可用时，回答追加"基于通用农业知识，知识库中暂无相关内容"标注，不阻塞问答
- 向量化 provider 切换：`.env.local` 的 `AI_EMBEDDING_PROVIDER`（mock|siliconflow）；真实模式下检索结果按语义相关性排序
